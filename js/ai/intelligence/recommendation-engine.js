...
//  Bu motor zaten KENDİ growth/trend hesabını YAPMIYORDU (Master Prompt'un
//  "Kendi growth hesabını yapmayacak" kuralı zaten karşılanıyordu) —
//  tek değişiklik, R1 bloğundaki doğrudan IMS erişiminin kaldırılmasıdır.
//
...
//  UI değişikliği: YOK
//
//  Bağımlılık: js/ai/core/ims-adapter.js, js/data/data-state.js, js/core/constants.js
//  Yükleme sırası: ims-adapter.js SONRASI
//  GitHub Pages compatible: classic script, no ES modules
// ══════════════════════════════════════════════════════════════════════
/* global GENEL, MIGI_BRICK_TL_RAW, ECZANE_RAW, eczaneLoaded */
...
(function() {
  ...
  function generateRecommendations(ttt, risks, opportunities, insights) {
    ...
    try {
      ...
      // üzerinden — bu motor zaten kendi growth/trend hesabını
      // YAPMIYORDU (Master Prompt'un "Kendi growth hesabını yapmayacak"
      // kuralı zaten karşılanıyordu); tek değişiklik, aşağıdaki R1
      // bloğundaki doğrudan IMS erişiminin kaldırılmasıdır.
      ...
      (risks || []).filter(function(r){ return r.severity === 'HIGH'; })
        ...
        .forEach(function(risk) {
          ...
          // ⚠️ AUDIT NOTU: bizim_pay alanı hiçbir gerçek veri kaynağında
          // yok (bkz. risk-engine.js R3 audit notu) — bu blok kasıtlı
          // olarak sessizce 0 eşleşme üretir (R3 zaten hiç "Pazar Payı"
          // başlıklı risk üretmediği için bu blok pratikte hiç tetiklenmez).
          if (risk.title.indexOf('Pazar Payı') !== -1) {
            var brickMatch = imsRecords.filter(function(r){
              return r.product && risk.title.indexOf(r.product) !== -1 &&
                     (r.bizim_pay || 0) < 15;
            });
  ...
  console.debug('[recommendation-engine] Phase 3.0 yüklendi (ims-adapter.js üzerinden).');
